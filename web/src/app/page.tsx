...
export default function Home() {
  ...
  const preview = useMemo(() => {
    ...
    const delayBase = 14 - capacity * 2.15;
    const delay = Math.max(1.8, delayBase - (policy === "fifo" ? 0 : priority / 90));
    ...
    return { workload, found, delay, coverage, review };
  ...
  return <main>
    ...
    <section id="lab" className="lab"><div className="section-head"><div><p className="section-label">INTERACTIVE POLICY LAB</p><h2>Change the policy. See the tradeoff.</h2></div><span className="sim-badge">SIMULATED PREVIEW</span></div><div className="lab-grid"><aside className="controls"><label>Daily inspection capacity <output>{capacity} teams</output><input type="range" min="1" max="6" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} /></label><label>Safety priority <output>{priority}%</output><input type="range" min="20" max="100" value={priority} onChange={(event) => setPriority(Number(event.target.value))} /></label><fieldset><legend>Policy posture</legend>{(Object.keys(policyCopy) as Policy[]).map((item) => <button key={item} className={policy === item ? "selected" : ""} onClick={() => setPolicy(item)}><span>{policyCopy[item].label}</span>{policyCopy[item].title}</button>)}</fieldset><div className="assumption"><b>What this preview assumes</b><p>Capacity and outcome behavior are scenario inputs. It is not a live city system or causal forecast.</p></div></aside><div className="results"><div className="result-title"><div><p>NOW VIEWING</p><h3>{policyCopy[policy].title}</h3><span>{policyCopy[policy].detail}</span></div><button className="review">Human review required</button></div><div className="metric-grid"><Metric label="Actionable findings" value={`${preview.found} / 16`} note="simulated discovery"/><Metric label="Average queue delay" value={`${preview.delay.toFixed(1)} days`} note="service responsiveness"/><Metric label="Coverage signal" value={`${preview.coverage.toFixed(0)}%`} note="operational group slice"/><Metric label="Cases for review" value={`${preview.review}`} note="not auto-decided"/></div><div className="queue-chart"><div className="chart-label"><span>Queue pressure across a 30-day scenario</span><b>Capacity {capacity} teams/day</b></div><svg viewBox="0 0 700 170" role="img" aria-label="Illustrative queue pressure curve"><path d="M0 135 C65 108 75 133 126 91 S208 100 263 56 S355 118 421 66 S530 74 578 39 S650 80 700 27" fill="none" stroke="currentColor" strokeWidth="4"/><path d="M0 135 C65 108 75 133 126 91 S208 100 263 56 S355 118 421 66 S530 74 578 39 S650 80 700 27 L700 170 L0 170Z" fill="currentColor" opacity=".08"/></svg><div className="chart-scale"><span>DAY 1</span><span>DAY 15</span><span>DAY 30</span></div></div><div className="action-log"><p>EXPLAINABLE ACTION LOG</p><div><span className="tag urgent">EXPEDITE</span><b>Repeat complaint, nighttime context</b><small>Human confirmation before dispatch</small></div><div><span className="tag routine">ROUTINE</span><b>Known permit overlaps complaint window</b><small>Keep visible; do not silently dismiss</small></div></div></div></div></section>
